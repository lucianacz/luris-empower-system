...
export async function GET() {
  ...
  const [accounts, transactions, questions, chains, imports, categories, investments] = await Promise.all([
    ...
    supabase.from("transactions").select("id,occurred_at,description,amount,currency,kind,status,excluded_from_totals,fee_amount,category_id,account:financial_accounts(name,institution),expense_splits(id,split_kind,label,percentage,amount)").eq("user_id", user.id).order("occurred_at", { ascending: false }).limit(250),
    ...
    supabase.from("categories").select("id,name,kind,color,icon,parent_id").eq("user_id", user.id).eq("is_archived", false).order("name"),
  ...
  const errors = [accounts.error, transactions.error, questions.error, chains.error, imports.error, categories.error, investments.error].filter(Boolean);
  if (errors.length) return Response.json({ error: errors[0]?.message ?? "Workspace data could not be loaded." }, { status: 422 });

  const totals = calculateTotals(transactions.data ?? []);
