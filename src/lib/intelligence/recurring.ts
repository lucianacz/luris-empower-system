...
export function analyzeRecurring(transactions: WorkspaceTransaction[], today: string) {
  ...
  for (const [key, rows] of groups) {
    ...
    const frequency = explicitlyMonthly ? "monthly" : medianGap >= 5 && medianGap <= 10 ? "weekly" : looksMonthly(dates, gaps) ? "monthly" : medianGap >= 320 && medianGap <= 410 ? "annual" : "uncertain";
    if (rows.length < 3 && frequency !== "annual") continue;
    ...
    const isSubscription = categoryName === "Subscriptions & software" || frequency === "monthly" && coefficientOfVariation(amounts) < 0.08;
    ...
    if (status !== "active" && frequency !== "uncertain") questions.push({ key: `recurring-stopped:${key}`, type: "recurring_may_have_stopped", prompt: `${pattern.providerName} may have stopped. Is it still active?`, explanation: `The last ${frequency} payment was ${lastPayment}.`, priority: 68, transactionIds: pattern.transactionIds.slice(-3), merchantKey: key });

    const thisMonthRows = sorted.filter((row) => row.occurred_at.startsWith(today.slice(0, 7)));
    if (categoryName === "Dining out" && thisMonthRows.length >= 3) insights.push({ key: `restaurant-visits:${key}:${today.slice(0, 7)}`, title: `${thisMonthRows.length} visits to ${pattern.providerName} this month`, body: `Open the ${thisMonthRows.length} supporting restaurant transactions.`, priority: 55, transactionIds: thisMonthRows.map((row) => row.id) });
  }
