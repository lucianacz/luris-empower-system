...
function TransactionRow({ transaction, categoryOptions: options, live, saveCategory, saveSplit }: { transaction: WorkspaceTransaction; categoryOptions: ReturnType<typeof categoryOptions>; live: boolean; saveCategory: (id: string, categoryId: string) => Promise<void>; saveSplit: (id: string, shared: boolean) => Promise<void> }) {
  ...
  return <tr className={crossedOut ? "align-top bg-[#faf7f3] opacity-60" : "align-top"}>
    ...
    <td className="px-4 py-3"><p className="mb-2 text-[10px] font-semibold text-[var(--muted)]">Paid by {transaction.paid_by?.display_name ?? transaction.account_owner?.display_name ?? "Luciana"}</p><div className="flex gap-1"><button disabled={!live} onClick={() => void saveSplit(transaction.id, false)} className={`rounded-lg border px-2 py-1 text-[10px] font-semibold disabled:opacity-45 ${(transaction.beneficiary_scope ?? "personal") === "personal" ? "border-[var(--forest)] bg-[var(--forest-soft)] text-[var(--forest)]" : "border-[var(--line)]"}`}>Luciana only</button><button disabled={!live} onClick={() => void saveSplit(transaction.id, true)} className={`rounded-lg border px-2 py-1 text-[10px] font-semibold disabled:opacity-45 ${transaction.beneficiary_scope === "shared" ? "border-[var(--amber)] bg-[var(--amber-soft)] text-[var(--amber)]" : "border-[var(--line)]"}`}>Shared 50/50</button></div></td>
  </tr>;
}
